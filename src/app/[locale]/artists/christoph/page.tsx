...
export default function ChristophPage() {
  const t = useIntlayer("christoph-page");
  const images = galleryImages.filter((img) => img.artist === "Christoph");

  const [selectedStyle, setSelectedStyle] = useState<"all" | "new-school">("all");

  const styleOptions: Array<{ key: "all" | "new-school"; label: string }> = [
    { key: "all", label: t.allStyles.value },
    { key: "new-school", label: t.newSchool.value },
  ];
